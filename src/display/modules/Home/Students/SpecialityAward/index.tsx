...
/**
 * SpecialityAwardSearch
 */
class SpecialityAwardSearch extends React.PureComponent<IProps, IState> {
  ...
  getList = async () => {
    ...
    const res = await students.getStudentSpecialityAwardList(params);
  ...
  */
  ...
  getParams = () => {
    ...
    if (this.state.filterParam) {
      ...
      if (this.state.filterParam.filter_title) {
        param.title = this.state.filterParam.filter_title;
      }
      if (this.state.filterParam.filter_prize_name) {
        param.prize_name = this.state.filterParam.filter_prize_name;
      }
      if (this.state.filterParam.filter_organzation) {
        param.organzation = this.state.filterParam.filter_organzation;
      }
    }
    return param;
  ...
  render() {
    ...
    const exportExcel = async () => {
      ...
      const res = await students.outputStudentSpecialityAwardExcel(params);
    ...
    */
    ...
    let column = [
      ...
      },
      {
        ...
        dataIndex: 'prize_name.prize_name',
        width: 200,
      },
      {
        title: '颁奖机构',
        ...
        render: (text: any, record: any) => {
          return (
            <span>
              {record.organizations.map((item: any) => (<Tag key={item.id} color="green">{item.organization_name}</Tag>))}
            </span>
          );
        }
      },
        ...
        <div className="layout-breadcrumb">
          ...
          <div className="page-button">
            ...
            <Select mode="multiple" placeholder="请选择筛选条件" className="filter-select" onChange={changeFilter}>
              ...
              <Option value={'filter_speciality_type'}>获奖类别</Option>
              <Option value={'filter_title'}>比赛名称</Option>
              <Option value={'filter_prize_name'}>获奖等级/荣誉称号</Option>
              <Option value={'filter_organzation'}>颁奖机构</Option>
            ...
            {/* 下面本页的筛选内容，根据筛选项选择显示 */}
            {
              ...
              <div className="filter-content">
                <Form className="filter-form" layout="inline" labelCol={{ span: 10 }} wrapperCol={{ span: 14 }}>
                  ...
                  {this.state.filterItems.indexOf('filter_title') >= 0 &&
                  ...
                  {this.state.filterItems.indexOf('filter_prize_name') >= 0 &&
                    <Form.Item label="获奖等级/荣誉称号">
                      {getFieldDecorator('filter_prize_name', {
                        rules: [],
                      })(
                        <Input placeholder="支持模糊匹配" />
                      )}
                  ...
                  {this.state.filterItems.indexOf('filter_organzation') >= 0 &&
                    <Form.Item label="颁奖机构">
                      {getFieldDecorator('filter_organzation', {
                        rules: [],
                      })(
                        <Input placeholder="支持模糊匹配" />
                      )}
...
export default Form.create({})(SpecialityAwardSearch);
