...
class AttachDrawer extends React.PureComponent<IProps, IState> {
  ...
  render() {
    const { drawerData, onClose, action, dir, changeFjdata, deleteFjdata } = this.props;
    // const { attachList } = this.state;
    console.log('接收的盗附件信息:', drawerData);
     ...
     * 删除附件
     */
    const del = async (uuuid: any) => {
      const res = await IPManage.deleteAttachesByUUUID(uuuid, {});
      if (res.code) {
    ...
    const onChange = async (info: any) => {
      ...
      if (info.file.status === 'done') {
        if (info.file.response.code) {
          // 如果上传成功
          const { id, file, filename } = info.file.response.results;
          const params = {
            uuid: id,
            dir,
            file_uri: file,
            filename,
            contract: drawerData.id,
          };
          console.log(params);
          // 调新建接口
          const res = await IPManage.postIPmanageAttaches(params);
          if (res.code) {
            message.success('上传成功!');
            changeFjdata(res.results);
            this.forceUpdate();
          } else {
            message.error(res.msg);
          }
    ...
    return (
      ...
      >
        <p style={{ ...pStyle, marginBottom: 24 }}>合同编号：{drawerData && drawerData.contract_code}的附件列表</p>
        <Divider />
        ...
        {/* <AttachList value={drawerData && drawerData.files} /> */}
        {
          drawerData
          && drawerData.attaches
          && drawerData.attaches.length > 0
          &&
          <List
            bordered={true}
            dataSource={drawerData.attaches}
            style={{ maxHeight: 600, overflowY: 'auto' }}
            renderItem={(item: any) => (
              <List.Item>
                  <Col span={10}>
                    <Typography.Text mark={true}>[附件]</Typography.Text><a href={item.file_uri} download={item.filename} target="_blank"> {item.filename} </a>
                  </Col>
                  <Col span={10}>
                    <Divider type="vertical" />
                    <span style={{ marginLeft: 30 }}>上传日期：{item.created}</span>
                  </Col>
                  <Col span={4}>
                    <Divider type="vertical" />
                    <span style={{ marginLeft: 30 }}>
                      <Popconfirm title="确认删除吗?" onConfirm={() => del(item.uuid)}>
