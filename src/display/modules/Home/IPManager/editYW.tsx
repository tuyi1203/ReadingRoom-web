...
import { Form, Input, Select, message, Switch, Icon, PageHeader, Divider, Button, Row, Modal, Tag } from 'antd';
 ...
 * Add
 */
class EditYW extends React.PureComponent<IProps, IState> {
  ...
  consistency = () => {
    ...
    let detail = this.state.detail;
    if (detail) {
      if (detail.broadcast_type !== detail.business_type) {
        flag = false;
      } else {
        if (typeof (detail.broadcastlocation) === 'number' && typeof (detail.businesslocation) === 'number') {
          if (detail.broadcastlocation === detail.businesslocation) {
            flag = true;
          }
        }
        if (typeof (detail.broadcastlocation) === 'object' && typeof (detail.businesslocation) === 'object') {
          if (detail.broadcastlocation && detail.businesslocation) {
            if (detail.broadcastlocation.sort().toString() === detail.businesslocation.sort().toString()) {
              flag = true;
            }
          }
        }
  ...
  render() {
    ...
    let detail = this.state.detail;
    console.log('编辑业务组件接收到的详情数据:', detail);
    ...
    //   dataCenterList.forEach((e: any) => {
    //     dataCenterListoptions.push(<Option value={e.city} key={e.city}>{e.nation}-{e.city}</Option>);
    //   });
    // }
            ...
            >
              ...
              <FormItem {...formItemLayout} label="业务类型">
                {getFieldDecorator('business_type', {
                  ...
                  <Select style={{ width: 450 }} onChange={(value: any) => { this.businesstypeChange(value); }}>
                    {IPBroadcastoptions}
                  </Select>
                )}
              </FormItem>
              ...
              <FormItem {...formItemLayout} label="一致性">
                {getFieldDecorator('consistency', {
                  ...
                  <Switch
                    ...
                    disabled={true}
                    // checked={(detail && (detail.business_location && detail.broadcast_location)) && detail.business_location.toString() === detail.broadcast_location.toString() ? true : false}
