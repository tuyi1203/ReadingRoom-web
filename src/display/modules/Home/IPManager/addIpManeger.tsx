...
import { Row, Col, Form, Input, InputNumber, Button, Select, message, Divider, Tabs, Switch, Icon, PageHeader, Drawer, Modal, Tag } from 'antd';
 ...
 * Add
 */
class AddIpManeger extends React.PureComponent<IProps, IState> {
  ...
  // 新建提交
  postAddIp = async () => {
    this.props.form.validateFields(['ip', 'netmask', 'contract'], async (err: any, value: any) => {
      ...
      let param = {
        ...
        segment_ips: this.getsegmentIps()
      };
      console.log('参数:', param);
      let res = await IPManage.postAddIp(param);
      if (!res.code) {
        message.error(res.msg);
        return;
      }
  ...
  render() {
    ...
    const { IpmanageContracts, IPType, IPBroadcast, ResourceType, nations, DomainListUnLoginList, moduleAction } = this.props.shareState;
    ...
    // const ownerdomainsoptions = (obj: any) => {
    //   let list: any = [];
    //   if (DomainListUnLoginList && DomainListUnLoginList.length > 0) {
    //     if (obj.resource_type === 2) {
    //       DomainListUnLoginList.forEach((e: any) => {
    //         if (!e.internal_domain) {
    //           list.push(<Option value={e.id} key={e.id}>{e.name}</Option>);
    //         }
    //       });
    //     }
    //     if (obj.resource_type === 3) {
    //       DomainListUnLoginList.forEach((e: any) => {
    //         list.push(<Option value={e.id} key={e.id}>{e.name}</Option>);
    //       });
    //     }
            ...
            < TabPane tab={v.parent ? <span><Icon type="pull-request" />{v.name}</span> : v.name} key={v.index} forceRender={true} >
              ...
              <FormItem {...formItemLayout} label="网关设备">
                {getFieldDecorator(`gateway_info${v.index}`, {
                  ...
                  <div>
                    {/* <Select style={{ width: 450 }} onChange={(value: any) => { this.gatewayinfoChange(value, v.name); }} mode={v.business_type === 2 ? 'multiple' : 'default'}>
                    {DatadictNetelementoptions(v)}
                  </Select> */}
                    {getGateWaytag(v)}
      ...
      <div style={{ width: '100%' }}>
        ...
        <div className="content">
          <div className="page-content">
            <PageHeader
              ghost={false}
              onBack={() => moduleAction.closeAdd()}
              title="新建"
            // subTitle="This is a subtitle"
            >
              <Divider orientation="left">合同IP</Divider>
              <Row>
                <Col span={12}>
                  <FormItem {...formItemLayout} label="IP段">
                    {getFieldDecorator('ip', {
                      initialValue: '24.2.0.0',
                      rules: [
                ...
                <Col span={12}>
                  <FormItem {...formItemLayout} label="子网掩码">
                    {getFieldDecorator('netmask', {
                      initialValue: 23,
                      rules: [
                        { required: true, message: '请填写子网掩码!' },
                      ],
                    })(
                      <InputNumber step={1} min={16} max={24} width={200} />
                    )}
                  </FormItem>
                </Col>
                <Col span={12}>
                  <FormItem {...formItemLayout} label="合同编号">
                    {getFieldDecorator('contract', {
                      ...
                      <Select style={{ width: 450 }}>
                        {IpmanageContractsOptions}
                      </Select>
                    )}
                  </FormItem>
                </Col>
                <Col span={12}>
                  <Button type="primary" style={{ width: 100, marginLeft: 170 }} onClick={() => { this.postIpsSplit(); }} >生成</Button>
                </Col>
              </Row>
              ...
              <Row style={{ textAlign: 'center' }}>
                <Button onClick={() => moduleAction.closeAdd()} >取消</Button>&nbsp;&nbsp;
          ...
          {this.state.plresourceType !== 1 && this.state.plresourceType !== null &&
            <FormItem {...formItemLayout} label="资源归属">
              {getFieldDecorator('plowner_domains', {
                initialValue: this.state.plowner_domains ? this.state.plowner_domains : [],
                rules: [
                  // { required: true, message: '请填写属性标识!' },
                ],
              })(
                <div>
                  {/* <Select style={{ width: 450 }} mode={'multiple'}>
                    {plownerdomainsoptions()}
                  </Select> */}
