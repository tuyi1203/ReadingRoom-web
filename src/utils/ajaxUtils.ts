...
var http = {
     ...
     * @param data 请求数据，JSON对象
     */
    postUpload: function (url: string, data: any): any {
        data = data || {};
        const newDate = new FormData();
        for (const [key, value] of Object.entries(data)) {
            newDate.append(key, value as any);
        }
        return new Promise((resolve, reject) => {
            _axios.post(url, newDate, {
                headers: getHeader(ContentType.MULTIPART),
